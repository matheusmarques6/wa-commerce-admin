...
interface Prompt {
  ...
  customer_thinking: string;
  customer_needs_to_hear: string;
  negative_instructions: string | null;
  false_beliefs: string[];
  dialog_examples: Array<{ cliente: string; agente: string }>;
}
interface Props { tenants: Tenant[]; prompt: Prompt }
...
export default function EditPromptModal({ tenants, prompt }: Props) {
  const [isOpen, setIsOpen] = useState(false)
  const [loading, setLoading] = useState(false)
  const [deleting, setDeleting] = useState(false)
  const [error, setError] = useState('')

  const [emotional, setEmotional] = useState(prompt.customer_emotional_state || '')
  const [thinking, setThinking] = useState(prompt.customer_thinking || '')
  const [needsHear, setNeedsHear] = useState(prompt.customer_needs_to_hear || '')
  const [negative, setNegative] = useState(prompt.negative_instructions || '')
  const [beliefs, setBeliefs] = useState<string[]>(prompt.false_beliefs?.length ? prompt.false_beliefs : [''])
  const [examples, setExamples] = useState<Array<{ cliente: string; agente: string }>>(prompt.dialog_examples?.length ? prompt.dialog_examples : [{ cliente: '', agente: '' }])

  useEffect(() => {
    if (isOpen) {
      setEmotional(prompt.customer_emotional_state || '')
      setThinking(prompt.customer_thinking || '')
      setNeedsHear(prompt.customer_needs_to_hear || '')
      setNegative(prompt.negative_instructions || '')
      setBeliefs(prompt.false_beliefs?.length ? prompt.false_beliefs : [''])
      setExamples(prompt.dialog_examples?.length ? prompt.dialog_examples : [{ cliente: '', agente: '' }])
    }
  }, [isOpen, prompt])
  ...
  return (
    <>
      <button
        ...
        className="text-xs font-semibold underline underline-offset-2 transition-colors hover:text-green-400"
        style={{ color: '#8888a0' }}
      >
        Editar
      </button>

      <Modal isOpen={isOpen} onClose={() => setIsOpen(false)} title="Editar Prompt">
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className={labelClass} style={labelStyle}>Loja</label>
              <select name="tenant_id" defaultValue={prompt.tenant_id} required className="w-full px-3 py-2 rounded-lg text-sm outline-none focus:ring-2" style={inputStyle}>
            ...
            <div>
              ...
              <select name="evento" defaultValue={prompt.evento || ''} required className="w-full px-3 py-2 rounded-lg text-sm outline-none focus:ring-2" style={inputStyle}>
          ...
          <div className="flex justify-between items-center mt-4">
            <button type="button" onClick={handleDelete} disabled={deleting} className="px-4 py-2 rounded-lg text-sm font-semibold transition-opacity" style={{ background: '#ef444418', color: '#ef4444', opacity: deleting ? 0.7 : 1 }}>
